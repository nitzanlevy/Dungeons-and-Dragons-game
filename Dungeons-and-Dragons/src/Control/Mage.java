...
public class Mage extends Player {
	...
	public void specialAbility() {
		...
		if (currentMana < cost)
			...
			List<Enemies> inRange = Controller.currentEnemies.stream()
					.filter(e -> position.dist(e.position)< range)
					.collect(Collectors.toList());
			
			while(hits < hitTimes & !inRange.isEmpty()) {
				int selected = Controller.random.nextInt(inRange.size());
				Enemies enemy = inRange.get(selected);
				
				int defense = Controller.random.nextInt(enemy.defensePoints);
				Controller.message += enemy.name + " rolled " + defense + " defense points.\n";
				int damage = Math.max(attackPoints - defense, 0);
				...
				if(enemy.getCurrentHealth() <= 0) {
					Controller.onEnemyDeath(this, enemy);
				}
			}
